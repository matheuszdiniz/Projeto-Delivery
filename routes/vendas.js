...
router.get("/", (req, res) => {
    Venda.find().lean().sort({ date: -1}).then(vendas => {
...
router.post('/nova', (req, res) => {
    ...
    function getDate(e) {
        ...
        if (dia < 10) dia = "0" + dia
        if (mes < 10) mes = "0" + mes
        if (e == 'agora') return `${dia}/${mes}/${agora.getFullYear()}`
        if (e == 'date') return `${agora.getFullYear()}/${mes}/${dia}`
    }
    ...
    if(!novaVenda.data || typeof novaVenda.data == undefined || novaVenda.data == null) {
        novaVenda.data = getDate('agora')
        novaVenda.date = new Date(getDate('date'))
    }else{
        novaVenda.date = new Date(`${novaVenda.data.split('-')[0]}-${novaVenda.data.split('-')[1]}-${Number.parseInt(novaVenda.data.split('-')[2])}`)
    ...
    if (erros.length > 0) {
        res.render("vendas/nova", { erros: erros })
    } else {
